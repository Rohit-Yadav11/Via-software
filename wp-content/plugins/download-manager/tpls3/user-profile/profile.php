...
<script>
    jQuery(function ($) {
        $('.profile-button.with-content').on('click', function () {
            ...
            $.get(wpdm_url.ajax, { action: 'wpdm_get_profile_menu_content', __pu: '<?php echo $user_ID; ?>', __pmenu: $(this).data('menu'), __scp: '<?php echo \WPDM\libs\Crypt::encrypt($params); ?>'}, function (response) {
        ...
        WPDM.blockUI('#wpdm-profile-contents');
        $.get(wpdm_url.ajax, { action: 'wpdm_get_profile_menu_content', __pu: '<?php echo $user_ID; ?>', __pmenu: (hash ? hash : '<?php echo $first_menu; ?>'), __scp: '<?php echo \WPDM\libs\Crypt::encrypt($params); ?>'}, function (response) {
            $('#wpdm-profile-contents').html(response);
