...
$_scparams =  \WPDM\libs\Crypt::encrypt($scparams);
?>
<style>
    ...
    <table class="table table-striped">
        ...
        <tr>
            <?php  foreach ($colheads as $index => $colhead){ ?>
                <th class="<?php echo $cols[$index] ?>"><?php echo $colhead; ?></th>
            <?php  } ?>
        </tr>
        </thead>
...
<script>
        ...
        paginate =  new Vue({
            ...
            methods: {
                nextPage: function(pageNum) {
                    ...
                    jQuery.get('<?php echo  home_url('wp-json/wpdm/v2/alldownloads'); ?>', {_scparams: scparams, cp:  pageNum}, function(response){
    ...
    jQuery.getScript('https://unpkg.com/vuejs-paginate@latest', function () {
    ...
    jQuery(function ($) {
        $.get('<?php echo  home_url('wp-json/wpdm/v2/alldownloads'); ?>', {_scparams: '<?php echo $_scparams;  ?>'}, function(response){
            datatable_<?php echo $scid;  ?>.packages = response.packages;
        ...
        $('#datatable_filter_<?php echo $scid ?>').submit(function (e) {
            ...
            WPDM.blockUI('#card_datatable_<?php echo $scid;  ?>');
            $(this).ajaxSubmit({
                url:  '<?php echo  home_url('wp-json/wpdm/v2/alldownloads'); ?>',
