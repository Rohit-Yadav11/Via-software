...
class Package {
    ...
    function prepare($ID = null, $template = null, $template_type = 'page')
    {
        ...
        if(self::templateHasTag($template, 'category')) {
        ...
        if(self::templateHasTag($template, array('file_type_icon', 'icon'))) {
            ...
            if (is_array($post_vars['files'])) {
             ...
             $post_vars['file_type_icon'] = '<img class="wpdm_icon" alt="' . __("Icon", "download-manager") . '" src="' . \WPDM\libs\FileSystem::fileTypeIcon((@count($post_vars['files']) <= 1 ? $ifn : 'zip')) . '" />';
        ...
        $loginmsg = \WPDM_Messages::login_required($post_vars['ID']);

        //Download limit is over
        if (self::userDownloadLimitExceeded($post_vars['ID'])) {
            $limit_over = 1;
            $limit_msg = \WPDM_Messages::download_limit_exceeded($post_vars['ID']);
    ...
    public static function fileCount($ID){
        ...
        $count = count(self::getFiles($ID, true));
        return $count;
     ...
     * @return array|mixed
     */
    public static function getFiles($ID, $include_dir = false){
    ...
    function prepareDownloadLink(&$package, $embed = 0, $extras = array())
    {
        ...
        if (!isset($package['quota']) || (isset($package['quota']) && $package['quota'] > 0 && $package['quota'] > $package['download_count']) || $package['quota'] == 0) {
            ...
            if (isset($package['tweet_lock']) && (int)$package['tweet_lock'] == 1) {
                $lock = 'locked';
                $sociallock .= \WPDM\PackageLocks::Tweet($package);
...
//                        $adata .= '<div class="modal fade" role="modal" id="pkg_' . $package['ID'] . "_" . $unqid . '"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><strong style="margin:0px;font-size:12pt">' . __('Download') . '</strong></div><div class="modal-body">' . $data . '</div><div class="modal-footer text-right"><button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Close</button></div></div></div></div>';
            ...
            $data = \WPDM_Messages::download_limit_exceeded($package['ID']);
        }
    ...
    public static function fetchTemplate($template, $vars, $type = 'link')
    {
        ...
        $loginmsg = \WPDM_Messages::login_required($vars['ID']);
        ...
        foreach ($igematches[0] as $nd => $scode) {
            $scode = str_replace(array("[", "]"), "", $scode);
            $vars[$scode] = \WPDM\libs\FileList::imageGallery($vars, $igematches[1][$nd], $igematches[2][$nd], $igematches[3][$nd]);
        }
    ...
    public static function fileTypes($ID, $img = true, $size = 16){
        ...
        $exico = '';
        foreach($ext as $exi){
            $exico .= "<img alt='{$exi}' title='{$exi}' class='ttip' style='width:{$size}px;height:{$size}px;' src='".\WPDM\libs\FileSystem::fileTypeIcon($exi)."' /> ";
        }
        if($img) return $exico;
        return $ext;
    }
     ...
     * @return string
     */
    public static function videoPreviewModal($package, $template_type = 'link')
    {
        if(is_int($package)) $package = get_post($package, ARRAY_A);
        $link_label = wpdm_valueof($package, 'link_label');
        $link_label = $link_label ? $link_label : self::linkLabel($package['ID']);
        $style = wpdm_download_button_style($template_type === 'page', $package['ID']);
        $files = isset($package['files']) ? $package['files'] : self::getFiles($package['ID']);
        $video = "";
        foreach ($files as $file){
     ...
     * @return mixed|string|void
     */
    public static function linkLabel($ID){
        $link_label =  get_post_meta($ID, '__wpdm_link_label',  true);
    ...
    function getThumbnail($ID, $FILEID, $size){
        ...
        if(in_array($ext, $imgext) && $abspath)
            $thumb = \WPDM\libs\FileSystem::imageThumbnail($abspath, $size[0], $size[1], WPDM_USE_GLOBAL, true);
        else if($ext === 'svg')
            $thumb = str_replace(ABSPATH, home_url('/'), $file);
        else if(strtolower($ext) === 'pdf' && class_exists('Imagick'))
            $thumb = \WPDM\libs\FileSystem::pdfThumbnail($file, md5($file));
        else {
            $thumb = \WPDM\libs\FileSystem::fileTypeIcon($ext);
        }
